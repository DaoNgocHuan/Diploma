...
void Diploma :: DrawResult()
{
	...
	//ui.K->setMaximum(IterationNumber);
	//QMessageBox msgBox;
	//msgBox.setText("The number of iterations is :" + QString::number(IterationNumber) + "\nThis number is greater than iteration number. Please choose another one");
	//msgBox.exec();
	//ui.K->setValue(IterationNumber);
	...
	//				scene->addRect(x,y,lengths[map[K][j]] * 3,20,pen);
	//				text = scene->addText("A"+QString::number(map[K][j] + 1), QFont("Arial", 7));
	//				text->setPos(x+(lengths[map[K][j]] * 3)/2 - 10, 0);
	//				x += lengths[map[K][j]] * 3;
	//			}
	//		}
	//	}
	//}
	//text = scene->addText("Put the segments together to form A:");
	//text->setPos(0,-50);
	//increase = 1;
...
void Diploma :: addFormular()
{
	...
	//pix.scaled(QSize(8,5),   Qt::IgnoreAspectRatio);
